...
$cod_tp_negociacao      = $_GET['cod_tp_negociacao'];
...
$sqlDeletaTpNegociacao = "DELETE FROM tipo_negociacao WHERE id = " . $cod_tp_negociacao . " AND (id_interno IS NULL OR id_interno = 0)";
echo $sqlDeletaTpNegociacao;

$resultadoDeleta = mysqli_query($conn, $sqlDeletaTpNegociacao);

if (mysqli_affected_rows($conn) > 0) {
    echo "<script>alert('Dados excluídos com sucesso!');</script>";
    header('location: ../../tipo_negociacao.php?exclusao=sucesso');
} else {
    echo "<script>alert('Não foi possível excluír o registro, pois já tem vinculo!')</script>";
    header('location: ../../tipo_negociacao.php?exclusao=erro');
}
} else {
    ...
    header('location: ../../tipo_negociacao.php?exclusao=erroP');
}
